...
const Profile: React.FC = () => {
    const { currentUser, logout, login, refreshUser } = useAuth();
    ...
    return (
        <div className="max-w-6xl mx-auto animate-fade-in pb-20 space-y-6">
            ...
            <main className="lg:col-span-9 space-y-6">
               ...
               {activeTab === 'personal' && (
                  <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm animate-fade-in transition-colors">
                        ...
                        onSubmit={async (e) => {
                           ...
                           const updates = {
                               ...
                               address: {
                                  ...
                                  city: (formData.get('cityState') as string).split('/')[0]?.trim() || '',
                                  state: (formData.get('cityState') as string).split('/')[1]?.trim() || '',
                           ...
                           try {
                               ...
                               // A função updateUser já exibe o toast de sucesso
                           } catch (err) {
                     ...
                     >
                        ...
                        <div className="pt-4">
                           <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100 mb-4 border-b border-slate-100 dark:border-slate-800 pb-2 transition-colors">Endereço de Cobrança</h3>
                           <div className="grid grid-cols-3 gap-4 mb-4">
                              <div className="col-span-1 space-y-1.5">
                                  ...
                                  <input name="zipCode" type="text" defaultValue={currentUser.address?.zipCode || ''} className="w-full h-11 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 font-bold text-sm text-slate-900 dark:text-slate-100 outline-none transition-colors font-sans" />
                              </div>
                              <div className="col-span-2 space-y-1.5">
                                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase transition-colors">Cidade / UF</label>
                                  <input name="cityState" type="text" defaultValue={currentUser.address ? `${currentUser.address.city} / ${currentUser.address.state}` : ''} placeholder="Ex: São Paulo / SP" className="w-full h-11 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 font-bold text-sm text-slate-900 dark:text-slate-100 outline-none transition-colors font-sans" />
                              </div>
                           </div>
                           <div className="space-y-1.5">
                               <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase transition-colors">Rua, Número e Bairro</label>
                               <input name="street" type="text" defaultValue={currentUser.address?.street || ''} className="w-full h-11 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 font-bold text-sm text-slate-900 dark:text-slate-100 outline-none transition-colors font-sans" />
                           </div>
                        </div>

                        <button type="submit" className="bg-slate-900 dark:bg-indigo-600 text-white px-8 py-3 rounded-xl text-xs font-black uppercase tracking-widest shadow-lg hover:shadow-indigo-500/20 hover:-translate-y-0.5 active:translate-y-0 transition-all mt-2">
                           Sincronizar Perfil
                        </button>
                     </form>
                  </div>
               ...
               {activeTab === 'billing' && (
                  <div className="space-y-6 animate-fade-in">
                     {/* Banner de Garantia Legal (7 Dias) */}
                     {!currentUser.subscription?.status || (currentUser.subscription?.status === 'active' && (() => {
                        const start = new Date(currentUser.subscription.current_period_start).getTime();
                        const now = new Date().getTime();
                        return (now - start) < (7 * 24 * 60 * 60 * 1000);
                     })()) ? (
                        <div className="bg-indigo-50 dark:bg-indigo-500/5 border border-indigo-100 dark:border-indigo-500/10 p-4 rounded-2xl flex items-center gap-4 animate-in slide-in-from-top-2 duration-500">
                           <div className="w-10 h-10 bg-indigo-600 rounded-full flex items-center justify-center shrink-0 shadow-lg shadow-indigo-500/20">
         ...
         {showCancelModal && currentUser.subscription && (
            <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md animate-in fade-in transition-all">
               <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden border border-rose-100 dark:border-rose-900/20 animate-in zoom-in slide-in-from-bottom-4 duration-300">
