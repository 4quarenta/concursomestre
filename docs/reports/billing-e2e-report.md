...
| --- | --- | --- | --- |
| Renovacao Stripe ponta a ponta | OK | Stripe Test Clock + createStripeInlineSubscription + finalizeStripeSubscription + runStripeReconciliationCron. | subscription=sub_1TJm5rHTtB22su0xLFxWeHPn<br>initial_invoice=in_1TJm5rHTtB22su0xPwQRchQv<br>renewal_invoice=in_1TJm65HTtB22su0xxB8l9Uff<br>transactions_after=2<br>synced_period_end=2026-06-07 23:43:43 |
| Auto renew off/on com backend como fonte final | OK | SubscriptionsService::updateRenewal + leitura remota da Stripe + verificacao de banco local. | off_message=Renovacao automatica desativada. A assinatura sera encerrada ao fim do periodo atual.<br>missing_card_error=Nenhum cartao padrao foi encontrado no Stripe para religar a renovacao automatica.<br>on_message=Renovacao automatica ativada com sucesso.<br>payment_method=pm_1TJm5nHTtB22su0xIOJEam7i |
| Upgrade e pro-rata canonico | OK | createStripeInlineSubscription + calculateSafeProratedCredit + persistencia de transacoes locais. | credit=19.33<br>coupon=E2EUP52678<br>upgrade_final=5.67<br>local_credit_mode=local_credit |
...
| Webhook duplicado | OK | Fixture invoice.paid + processStripeWebhookEventObject com Stripe real e banco local. | event_id=evt_dup_bd9df505<br>first_duplicate={"received":true}<br>second_duplicate={"received":true,"duplicate":true}<br>transactions=1 |
...
| Webhook atrasado + reconciliacao | OK | Stripe Test Clock + runStripeReconciliationCron + fixture invoice.paid atrasado. | renewal_invoice=in_1TJm7hHTtB22su0xRDUsXY3J<br>recovery_transactions=2<br>delayed_result={"received":true} |
