 ...
 */
function fingerprintAutomaticPayload(payloads: GranImportPayload[]): string {
  const source = JSON.stringify(payloads);
  let hash = 0x811c9dc5;
...
const AdminGranCrawlerSection = ({
  ...
  const handlePurgeFailureDiagnostics = React.useCallback(async () => {
    ...
    const confirmed = window.confirm(
      `Limpar ${totalEligible} diagnostico(s) encerrado(s)? `
      + `Snapshots com mais de ${payloadDays} dias e registros com mais de ${recordDays} dias serao removidos. `
      + 'Falhas abertas, novas tentativas e questoes publicadas nao serao alteradas.',
    );
    if (!confirmed) return;
  ...
  const waitForPublicationBatch = React.useCallback(async (
    ...
    while (['pending', 'processing'].includes(batch.status)) {
      ...
      const response = await apiClient.post(ENDPOINT, {
        action: 'publication_batch_status',
        batchId: batch.batchId,
      }, { signal });
      batch = readApiData<GranPublicationBatch>(response);
      setCurrentBatch(batch);
    }
    return batch;
  }, []);
  ...
  const startAutomaticMode = React.useCallback(async () => {
    ...
    try {
      ...
      while (!controller.signal.aborted && cursorYear <= finalYear) {
        ...
        let data: GranFetchResult | null = null;
        for (let attempt = 1; !controller.signal.aborted && attempt <= 5; attempt += 1) {
          try {
            data = await collectMappedPage(cursorPage, String(cursorYear), controller.signal);
            break;
          } catch (requestError) {
        ...
        if (!data) break;
        setResult(data);
        const pagePayloads = data.payloads || [];
        const pageQuestionCount = countGranPayloadQuestions(pagePayloads);
        ...
        if (pageQuestionCount > 0) {
          ...
          const response = await apiClient.post(ENDPOINT, {
            ...
            idempotencyKey: `gran-auto-${checkpoint.runKey}-${cursorYear}-${cursorPage}-${fingerprintAutomaticPayload(pagePayloads)}`,
      ...
      <section className={`${ADMIN_PAGE_PANEL_CLASS} space-y-4`} data-testid="gran-publication-failure-history">
        ...
        <div className="flex flex-col gap-3 rounded-md border border-slate-200 bg-slate-50 px-4 py-3 text-xs dark:border-slate-700 dark:bg-slate-950/30 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => void handlePurgeFailureDiagnostics()}
