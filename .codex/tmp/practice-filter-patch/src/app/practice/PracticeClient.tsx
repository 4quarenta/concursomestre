...
import { buildQuestionBankFilterSignature } from '@/state/question-bank/questionBankQuery';
...
const Practice: React.FC<PracticeProps> = ({ initialQuestionPage }) => {
  ...
  const currentServerFilterSignature = useMemo(
    () => buildQuestionBankFilterSignature(questionQueryParams),
    [questionQueryParams],
  );
  const isCurrentServerFilteredResult = isQuestionsLoaded
    && loadedQuestionBankFilterSignature === currentServerFilterSignature;
  ...
  const filteredQuestions = useMemo(() => {
    let filtered = questions.filter(q => {
      ...
      // The API has already applied these filters. Reapplying them in the
      // browser used to hide valid results whenever legacy and canonical
      // taxonomy DTOs used different field names. ENEM remains local because
      // it is a virtual focus assembled by the client.
      const applyLocalServerPredicates = !isCurrentServerFilteredResult || isEnemFocus;
    ...
    return filtered;
  }, [filters, questions, currentUser?.savedQuestionIds, userAnswers, highlightedQuestionId, isCurrentServerFilteredResult, scopedQuestionIdSet]);
