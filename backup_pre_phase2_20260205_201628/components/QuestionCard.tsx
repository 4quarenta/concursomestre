...
import { Question, UserAnswer, Comment, ErrorReport, UserNote, QuestionStats } from '../types';
import {
  CheckCircle2, XCircle, Flag, BookOpen, GraduationCap,
  Eye, EyeOff, Building2, Calendar, Briefcase, ThumbsUp, MessageSquare, BarChart3, AlertTriangle, Share2, Lock, StickyNote, Bookmark, BookmarkCheck, ChevronDown, ChevronUp, Layers, Tag, Crown, Zap, Star, Reply, History
} from 'lucide-react';
...
const QuestionCard: React.FC<QuestionCardProps> = ({
  ...
  const { systemSettings, fetchComments, reportComment } = useData();
  ...
  const isElite = userPlan === 'Elite' || userPlan === 'Pro';
  const canSeeDetailed = userPlan !== 'Gratuito';
  ...
  const handleToggleStats = async () => {
    ...
    if (nextState) {
      ...
      try {
        const res = await api.get<any>('api/questions/get_stats.php', { question_id: String(question.id) });
  ...
  useEffect(() => {
    if (showComments && (!question.comments || question.comments.length === 0)) {
  ...
  useEffect(() => {
    if (isHistoryModalOpen) {
      ...
      api.get<UserAnswer[]>('api/questions/history.php', { question_id: String(question.id), user_id: currentUser?.id || '' })
        .then(res => {
  ...
  const handleShare = () => {
    const textToShare = `Questão: ${question.enunciado_clean || '...'}\n\n${(question.itens || []).map((item, i) => `${item.rotulo}) ${item.corpo_clean || '...'}`).join('\n')}\n\nConcursoMestre AI`;
    if (navigator.share) {
      navigator.share({
  ...
  return (
    <div id={String(question.id)} className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm w-full overflow-hidden flex flex-col transition-all relative">
      ...
      <div className="p-6 space-y-6">
        ...
        {question.introText && (
          <div className="bg-slate-50 dark:bg-slate-800/50 border-l-2 border-indigo-200 dark:border-indigo-800 p-4 rounded-r-xl">
        ...
        <div className="space-y-4">
          <div className="text-lg text-slate-800 dark:text-slate-100 font-bold leading-relaxed prose prose-indigo dark:prose-invert max-w-none">
          ...
          {question.imageUrl && (
            <div className="my-4 rounded-xl overflow-hidden border border-slate-100 dark:border-slate-800">
              <img src={question.imageUrl} alt="Anexo" className="max-w-full h-auto mx-auto max-h-[400px]" />
            </div>
          )}
        </div>
        ...
        <div className="px-6 py-4 bg-slate-50/50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex flex-wrap gap-3 items-center justify-between transition-colors duration-300">
          <div className="flex gap-2 items-center flex-wrap">
            {question.teacherComment && (
              <button
                onClick={() => { setShowTeacherComment(!showTeacherComment); setShowDetailedComment(false); }}
                className={`flex items-center gap-1.5 font-bold text-[9px] uppercase px-3 py-2 rounded-lg border transition-all ${isElite ? 'text-amber-700 dark:text-amber-400 bg-white dark:bg-slate-700 border-amber-200 dark:border-amber-800 hover:bg-amber-50 dark:hover:bg-slate-600' : 'text-slate-400 bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700'}`}
              >
                {isElite ? <GraduationCap size={14} /> : <Lock size={12} />}
                Gabarito Comentado
              </button>
            )}

            {question.detailedComment && systemSettings.features.aiCommentsEnabled && (
              <button
                onClick={() => { if (canSeeDetailed) { setShowDetailedComment(!showDetailedComment); setShowTeacherComment(false); } else { alert("Recurso exclusivo para assinantes!"); } }}
                className={`flex items-center gap-1.5 font-bold text-[9px] uppercase px-3 py-2 rounded-lg border transition-all ${canSeeDetailed ? 'text-indigo-700 dark:text-indigo-300 bg-white dark:bg-slate-700 border-indigo-200 dark:border-indigo-800 hover:bg-indigo-50 dark:hover:bg-slate-600' : 'text-slate-400 bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700'}`}
              >
            ...
            {systemSettings.features.communityEnabled && (
              <button onClick={() => setShowComments(!showComments)} className={`flex items-center gap-1.5 px-3 py-2 rounded-lg font-bold text-[9px] uppercase border transition-all ${showComments ? 'bg-indigo-600 text-white border-indigo-600 shadow-sm' : 'bg-white dark:bg-slate-700 text-slate-500 dark:text-slate-300 border-slate-200 dark:border-slate-600 hover:border-indigo-300'}`}>
                <MessageSquare size={14} /> {question.commentsCount || 0} Comentários
              </button>
            )}

            <button onClick={handleToggleStats} className={`flex items-center gap-1.5 px-3 py-2 rounded-lg font-bold text-[9px] uppercase border transition-all ${showStats ? 'bg-indigo-600 text-white border-indigo-600 shadow-sm' : 'bg-white dark:bg-slate-700 text-slate-500 dark:text-slate-300 border-slate-200 dark:border-slate-600 hover:border-indigo-300'}`}>
        ...
        {/* Áreas Expandidas */}
        {(showComments || showTeacherComment || showDetailedComment || showStats) && (
          <div className="bg-slate-50/50 dark:bg-slate-900/50 border-t border-slate-200 dark:border-slate-800 animate-fade-in divide-y divide-slate-100 dark:divide-slate-800">
            ...
            {showTeacherComment && question.teacherComment && (
            ...
            {showDetailedComment && question.detailedComment && (
...
export default React.memo(QuestionCard, (prevProps, nextProps) => {
  ...
  return (
    ...
    prevProps.existingAnswer === nextProps.existingAnswer &&
