...
import {
   ...
   Info, Calendar, Save, X, ArrowRight, Settings, LayoutGrid, List, Search, Clock, AlertCircle, Edit3, Trash2, CheckCircle, ShieldCheck, Hash, Layers, UserCheck, FileText, Check, PlusCircle
} from 'lucide-react';
...
const RankingPage: React.FC = () => {
   ...
   const { rankings, addRanking, updateRanking, deleteRanking, submitRankingEntry } = useData();
   ...
   const [authModalConfig, setAuthModalConfig] = useState({ title: '', description: '', actionSource: 'ranking' });

   // Forms
   const [rankingForm, setRankingForm] = useState({
      name: '', institution: '', totalQuestions: 60, vacancies: 10, officialKeyReleaseDate: '',
      keyStatus: 'pending' as 'official' | 'pending', hasDiscursive: false, examTypes: [] as string[]
   });
   const [newTypeName, setNewTypeName] = useState('');

   const [partForm, setPartForm] = useState({
      registration: '', examType: '', category: 'AC' as 'AC' | 'Afro' | 'PCD', discursiveScore: 0, answers: [] as string[]
   });
   ...
   const handleCreateOrUpdate = (e: React.FormEvent) => {
      ...
      if (isEditing && selectedRanking) {
         const updated = { ...selectedRanking, ...rankingForm };
   ...
   const handleFinishParticipation = () => {
      ...
      const e: RankingEntry = {
         ...
         userAnswers: partForm.answers.join(''),
         score: 0,
         timestamp: Date.now(),
         status: 'active'
      };
      const r = { ...selectedRanking, entries: [...selectedRanking.entries, e] };
   ...
   return (
      <div className="max-w-6xl mx-auto space-y-6 animate-fade-in pb-32">
            ...
            <>
               <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                  <div>
                     ...
                     <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Classificação colaborativa e nota de corte projetada.</p>
                  </div>
                  ...
                  <div className="flex gap-2">
                     <button onClick={() => {
                        ...
                        setRankingForm({ name: '', institution: '', totalQuestions: 60, vacancies: 10, officialKeyReleaseDate: '', keyStatus: 'pending', hasDiscursive: false, examTypes: [] }); setIsCreating(true);
            ...
            <div className="space-y-6 animate-slide-up">
               <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  ...
                  <div className="flex items-center gap-3">
                        ...
                        <div className="flex gap-2">
                           <button onClick={() => {
                              setRankingForm({
                                 ...
                                 vacancies: selectedRanking.vacancies,
                     ...
                     {selectedRanking.officialKeyReleaseDate && (
               ...
               <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row gap-8 relative overflow-hidden transition-colors">
                  <div className="flex-1 space-y-4">
                     ...
                     <div className="flex flex-wrap gap-8 pt-2">
                        <div className="space-y-0.5"><p className="text-[9px] font-black text-slate-300 dark:text-slate-600 uppercase tracking-widest transition-colors">Nota de Corte</p><p className="text-2xl font-black text-indigo-600 dark:text-indigo-400">{processedData?.entries[selectedRanking.vacancies - 1]?.score || '--'}</p></div>
                        ...
                        <div className="space-y-0.5"><p className="text-[9px] font-black text-slate-300 dark:text-slate-600 uppercase tracking-widest transition-colors">Vagas</p><p className="text-2xl font-black text-slate-900 dark:text-slate-100 transition-colors">{selectedRanking.vacancies}</p></div>
                     </div>
                  </div>
                  <div className="flex items-center md:items-end">
                     <button onClick={() => {
                        if (!currentUser) {
                           setAuthModalConfig({
                              title: "Participe do Ranking",
                              description: "Para enviar seu gabarito e ver sua colocação, você precisa estar logado.",
                              actionSource: 'ranking_participate'
                           });
                           setShowAuthModal(true);
                           return;
                        }
                        setParticipationStep(1);
                        setPartForm({ registration: '', examType: selectedRanking.examTypes[0] || '', category: 'AC', discursiveScore: 0, answers: Array(selectedRanking.totalQuestions).fill('') });
                     ...
                     <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-slate-50 dark:divide-slate-800 text-xs">
                           {processedData?.entries.map((e, idx) => {
                              const inVagas = idx < selectedRanking.vacancies;
                              const inCR = idx < selectedRanking.vacancies + selectedRanking.reserveLimit;
                              return (
         ...
         {(isCreating || isEditing) && (
            <div className="fixed inset-0 bg-white/90 dark:bg-slate-950/90 backdrop-blur-sm z-50 flex items-center justify-center p-6 transition-colors">
               <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-[2.5rem] border border-slate-200 dark:border-slate-800 p-8 shadow-2xl animate-scale-in max-h-[90vh] overflow-y-auto no-scrollbar transition-colors">
                  ...
                  <form onSubmit={handleCreateOrUpdate} className="space-y-5">
                     ...
                     <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase ml-1 transition-colors">Vagas</label><input required type="number" value={rankingForm.vacancies} onChange={e => setRankingForm({ ...rankingForm, vacancies: Number(e.target.value) })} className="w-full h-11 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 font-bold text-slate-900 dark:text-slate-100 text-xs outline-none transition-colors" /></div>
                        <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase ml-1 transition-colors">Qtd. Questões</label><input required type="number" value={rankingForm.totalQuestions} onChange={e => setRankingForm({ ...rankingForm, totalQuestions: Number(e.target.value) })} className="w-full h-11 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 font-bold text-slate-900 dark:text-slate-100 text-xs outline-none transition-colors" /></div>
                     </div>
                     ...
                     <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase ml-1 transition-colors">Gabarito</label>
                           <select value={rankingForm.keyStatus} onChange={e => setRankingForm({ ...rankingForm, keyStatus: e.target.value as any })} className="w-full h-11 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 font-bold text-slate-900 dark:text-slate-100 text-xs transition-colors">
                              <option value="pending">Presumido</option>
                              <option value="official">Oficial</option>
                           </select>
                        </div>
                        <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase ml-1 transition-colors">Data Oficial</label><input required type="date" value={rankingForm.officialKeyReleaseDate} onChange={e => setRankingForm({ ...rankingForm, officialKeyReleaseDate: e.target.value })} className="w-full h-11 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 font-bold text-slate-900 dark:text-slate-100 text-xs outline-none transition-colors" /></div>
                     </div>
         ...
         {isParticipating && selectedRanking && (
            <div className="fixed inset-0 bg-slate-900/80 dark:bg-slate-950/95 backdrop-blur-sm z-50 flex items-center justify-center p-6 transition-colors">
                        ...
                        <div className="space-y-6 animate-fade-in">
                           <div className="grid grid-cols-2 gap-4">
                              ...
                              <div className="space-y-1">
                                 ...
                                 >
                                    ...
                                    {selectedRanking.examTypes.map((type, i) => <option key={i} value={type}>{type}</option>)}
                           ...
                           <div className="space-y-3">
                              ...
                              <div className="grid grid-cols-3 gap-2">
                                 {[{ id: 'AC', label: 'AC' }, { id: 'Afro', label: 'Afro' }, { id: 'PCD', label: 'PCD' }].map(cat => (
                                    <button key={cat.id} type="button" onClick={() => setPartForm({ ...partForm, category: cat.id as any })} className={`py-2.5 rounded-xl border text-[10px] font-black uppercase transition-all ${partForm.category === cat.id ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-slate-50 dark:bg-slate-800 border-slate-100 dark:border-slate-700 text-slate-400 dark:text-slate-500 hover:border-indigo-200'}`}>{cat.label}</button>
                                 ))}
                              </div>
                           </div>
                           ...
                           {selectedRanking.hasDiscursive && (
                              <div className="space-y-1">
                                 <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1 flex items-center gap-1.5 transition-colors"><FileText size={12} className="text-indigo-500 dark:text-indigo-400" /> Nota Discursiva (0-100)</label>
