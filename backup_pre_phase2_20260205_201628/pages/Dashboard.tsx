...
const Dashboard: React.FC = () => {
  ...
  const { userAnswers: answers, questions } = useData();
  ...
  const subjectData = useMemo(() => {
    ...
    filteredAnswers.forEach(ans => {
      ...
      if (question) {
        if (!stats[question.subject]) stats[question.subject] = { total: 0, acertos: 0 };
        stats[question.subject].total += 1;
        if (ans.isCorrect) stats[question.subject].acertos += 1;
      }
    });
  ...
  const userCommentsCount = useMemo(() => {
    ...
    const checkComments = (commentList: any[]) => {
      commentList.forEach(c => {
        const commentTime = new Date(c.date).getTime();
        if (c.userId === currentUser.id && (timeRange === 'all' || commentTime >= limit)) {
          count++;
        }
        if (c.replies && c.replies.length > 0) {
          checkComments(c.replies);
        }
      });
    };
    ...
    questions.forEach(q => {
      if (q.comments) {
        checkComments(q.comments);
      }
    });

    return count;
  }, [questions, currentUser, timeRange]);
