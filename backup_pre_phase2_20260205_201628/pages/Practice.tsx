...
const Practice: React.FC = () => {
  ...
  const [filters, setFilters] = useState({
    keyword: '', subject: 'All', difficulty: 'All', agency: 'All', year: 'All', level: 'All', topic: 'All', role: 'All', modality: 'All',
    onlySaved: false, hasTeacherComment: false, hasDetailedComment: false,
    excludeCanceled: false, excludeOutdated: false, excludeAnswered: false
  });
  const [pendingFilters, setPendingFilters] = useState(filters); // State for UI selection before submit
  ...
  // URL query parameter for highlighting specific question
  ...
  const filteredQuestions = useMemo(() => {
    let filtered = questions.filter(q => {
      ...
      const matchDifficulty = filters.difficulty === 'All' || q.dificuldade === (filters.difficulty === 'Fácil' ? 1 : (filters.difficulty === 'Médio' ? 2 : 3));
      ...
      return matchSubject && matchDifficulty && matchKeyword && matchAgency && matchYear && matchLevel && matchTopic && matchRole && matchModality && matchSaved && matchTeacher && matchDetailed && matchCanceled && matchOutdated && matchExcludeAnswered;
    });
  ...
  const handleAnswer = useCallback((ans: UserAnswer) => {
    ...
    dispatchAnswer(ans);
    addXp(ans.isCorrect ? 10 : 2);
  }, [currentUser, dispatchAnswer, addXp]);
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-5 transition-colors duration-300">
          <div className="flex gap-2">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => {
                  const reset = { keyword: '', subject: 'All', difficulty: 'All', agency: 'All', year: 'All', level: 'All', topic: 'All', role: 'All', modality: 'All', onlySaved: false, hasTeacherComment: false, hasDetailedComment: false, excludeCanceled: false, excludeOutdated: false, excludeAnswered: false };
                  setFilters(reset);
            ...
            <div className="relative min-h-[400px]">
              ...
              <div className="animate-slide-up">
                <QuestionCard
                  ...
                  existingNote={userNotes.find(n => n.questionId === filteredQuestions[currentQuestionIndex].id)}
          ...
          <div className="space-y-4">
            {paginatedList.map((q, i) => (
              <QuestionCard
                ...
                existingNote={userNotes.find(n => n.questionId === q.id)}
            ...
            <div ref={loaderRef} className="h-10 flex items-center justify-center">
              {visibleCount < filteredQuestions.length && <Loader2 className="animate-spin text-indigo-400" size={24} />}
            </div>
          </div>
        )}
      </div>
