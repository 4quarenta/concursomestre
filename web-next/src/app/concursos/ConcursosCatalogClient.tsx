...
type ConcursosCatalogClientProps = {
...
const buildFallbackCards = (
  ...
  return safeAgencies.map((agency, agencyIndex) => {
...
export default function ConcursosCatalogClient({ taxonomies }: ConcursosCatalogClientProps) {
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedYear, setSelectedYear] = useState('Todos');

  const agencies = useMemo(
    () => (taxonomies.agencies || []).map((item) => item.name).filter(Boolean),
    [taxonomies.agencies],
  );
  const roles = useMemo(
    () => (taxonomies.roles || []).map((item) => item.name).filter(Boolean),
    [taxonomies.roles],
  );
  const years = useMemo(
    () => (taxonomies.years || []).map(String).filter(Boolean).sort().reverse(),
    [taxonomies.years],
  );
        ...
        <div className="grid gap-0 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900 px-6 py-8 text-white md:px-8">
            ...
            <div className="mt-4 flex items-start gap-4">
              ...
              <div>
                <h1 className="text-3xl font-black tracking-tight text-white md:text-5xl">Concursos</h1>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-2 text-sm font-medium leading-6 text-slate-600 dark:text-slate-300">
                A proxima etapa natural e vincular editais, materiais, rankings e paginas publicas por concurso, mantendo SEO e navegacao canonica.
              </p>
            </div>
          </div>
        </div>
