...
const Support: React.FC = () => {
    ...
    const fetchHistory = async () => {
        try {
            const res: any = await apiClient.get('feedback/list.php');
            if (res.success) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res: any = await apiClient.post('feedback/create.php', payload);
    ...
    const toggleFeedback = async (id: number) => {
        ...
        if (!replies[id]) {
            ...
            try {
                const res: any = await apiClient.get(`feedback/list.php?id=${id}`);
                        ...
                        <div key={item.id} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                            ...
                            {expandedFeedbackId === item.id && (
                                <div className="bg-slate-50 dark:bg-slate-950/50 border-t border-slate-100 dark:border-slate-800 p-4 space-y-3">
                                    ...
                                    {/* Optional: Add Reply Form Here if users can reply back */}
                                    <div className="pt-2">
                                        <form onSubmit={(e) => {
                                            e.preventDefault();
                                            // Handle reply submission logic here if needed
                                            // For now, simpler implementation as requested just shows conversation
                                            addToast('Para continuar a conversa, chame no WhatsApp.', 'info');
                                            ...
                                            <input
                                                ...
                                                readOnly // Readonly for now until full chat logic
                                                onClick={() => addToast('Funcionalidade de resposta direta em breve!', 'info')}
                                            />
                                            <button type="button" className="p-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors">
                                                <Send size={14} />
                                            </button>
                                        </form>
                                    </div>
