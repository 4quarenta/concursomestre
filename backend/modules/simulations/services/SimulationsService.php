...
class SimulationsService
{
    ...
    /**
     * Persiste o simulado do usuario autenticado.
     * Pontuacao e correcao sao sempre calculadas pelo servidor com o gabarito persistido.
     */
    public function saveSimulation(array $payload, array $authenticatedUserPayload): array
    {
        ...
        $configQuestionIds = is_array($config['questionIds'] ?? null)
            ? array_values(array_filter(array_map('intval', $config['questionIds'])))
            : [];
        $answerQuestionIds = array_values(array_filter(
            array_map('intval', array_keys($normalized['answers'])),
        ...
        foreach ($normalized['answers'] as $questionId => $answerPayload) {
            ...
            if (!array_key_exists($questionKey, $correctOptionIndexes)) {
                throw new InvalidArgumentException('Nao foi possivel validar o gabarito da questao ' . $questionKey . '.');
            }
    ...
    /**
     * Normaliza somente dados enviados pelo usuario. `is_correct` e ignorado deliberadamente.
     */
    private function normalizeAnswerPayload(mixed $answerPayload): array
    {
        if (is_array($answerPayload)) {
            return [
                ...
                'time_taken_seconds' => isset($answerPayload['time_taken']) ? max(0, (int) $answerPayload['time_taken']) : 0,
            ];
        }
