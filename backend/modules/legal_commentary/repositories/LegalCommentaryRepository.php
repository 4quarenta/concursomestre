...
class LegalCommentaryRepository
{
    ...
    public function fetchLawDetail(string $identifier, ?string $userId = null, bool $incrementAccess = false): ?array
    {
        ...
        $progressByLaw = $this->fetchProgressByLaw($userId);
        $law = $this->mapLawSummary($row, $favorites, $progressByLaw);
        $sectionRows = $this->fetchSectionRows((int) $row['id']);
        $articleRows = $this->fetchArticleRows((int) $row['id']);
        $articleIds = array_map(fn ($article) => (string) $article['id'], $articleRows);
        $progress = $progressByLaw[(string) $row['id']] ?? null;

        $articleBlockRows = $this->fetchArticleBlockRows($articleIds);
        $teacherRows = $this->fetchRowsForArticles('teacher_comments', $articleIds);
        $jurisprudenceRows = $this->fetchRowsForArticles('article_jurisprudence', $articleIds);
        $examTipRows = $this->fetchRowsForArticles('article_exam_tips', $articleIds);
        $sumulaRows = $this->fetchRowsForArticles('article_sumulas', $articleIds);
        $blocksByArticle = $this->groupRowsByColumn($articleBlockRows, 'law_article_id');
        $teacherByArticle = $this->groupRowsByColumn($teacherRows, 'law_article_id');
        $jurisprudenceByArticle = $this->groupRowsByColumn($jurisprudenceRows, 'law_article_id');
        $examTipsByArticle = $this->groupRowsByColumn($examTipRows, 'law_article_id');
        $sumulasByArticle = $this->groupRowsByColumn($sumulaRows, 'law_article_id');
        ...
        return array_merge($law, [
            ...
            'sectionEditorials' => array_map(
                ...
                $this->fetchSectionEditorialRows((int) $row['id'])
            ),
        ]);
    }
    ...
    private function fetchArticleRows(int $lawId): array
    {
        $stmt = $this->db->prepare(
    ...
    private function fetchSectionRows(int $lawId): array
    {
        if ($lawId <= 0) {
            return [];
        }
    ...
    private function fetchSectionAssuntoFilterMap(int $lawId): array
    {
        if ($lawId <= 0) {
            return [];
        }
    ...
    private function fetchArticleBlockRows(array $articleIds): array
    {
        if (empty($articleIds)) {
            return [];
        }
    ...
    private function fetchRowsForArticles(string $table, array $articleIds): array
    {
        if (empty($articleIds)) {
            return [];
        }
    ...
    private function fetchSectionEditorialRows(int $lawId): array
    {
        if ($lawId <= 0) {
            return [];
        }
    ...
    private function fetchBatchArticleRows(int $lawId, array $articleIds = []): array
    {
        if ($lawId <= 0) {
            return [];
        }
    ...
    private function fetchGroupedRows(string $table, string $column, array $ids): array
    {
        if (empty($ids)) {
            return [];
        }
    ...
    public function fetchArticleEditorialSnapshot(int $lawId, int $articleId): array
    {
        ...
        $favorites = [];
        $teacherRows = $this->fetchRowsForArticles('teacher_comments', [(string) $articleId]);
        $jurisprudenceRows = $this->fetchRowsForArticles('article_jurisprudence', [(string) $articleId]);
        $examTipRows = $this->fetchRowsForArticles('article_exam_tips', [(string) $articleId]);
        $sumulaRows = $this->fetchRowsForArticles('article_sumulas', [(string) $articleId]);
    ...
    private function syncArticles(int $lawId, array $articles, array $sectionIdMap = [], bool $preserveUnmatched = false): array
    {
        ...
        $seenPayloadSlugs = [];
        $existingBySlug = [];
        $sectionAssuntoFilterMap = $this->fetchSectionAssuntoFilterMap($lawId);
