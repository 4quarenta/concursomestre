...
require_once __DIR__ . '/../../modules/seo/reports/ProfessionalTaxonomyReadinessReporter.php';
...
try {
    ...
    $report = (new ProfessionalTaxonomyReadinessReporter($database->getConnection()))->generate();
    echo json_encode($report, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR) . PHP_EOL;
    exit(0);
} catch (Throwable $error) {
    fwrite(STDERR, 'Professional taxonomy readiness report nao executado: ' . $error->getMessage() . PHP_EOL);
    exit(2);
}
