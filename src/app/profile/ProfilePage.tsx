...
import SavedQuestionsPanel from './components/SavedQuestionsPanel';
...
const Profile: React.FC = () => {
   ...
   const confirm = useConfirm();
    ...
    React.useEffect(() => {
        ...
        void ensureTaxonomiesLoaded()
            .then(() => {
                if (isActive) setStudyFocusLoadStatus('ready');
            })
            .catch((error) => {
                clientLog.warn('profile.study_focus_taxonomies.load_failed', error);
                if (isActive) setStudyFocusLoadStatus('error');
            });
        ...
        <div className="space-y-5">
            {billingPaymentIssue && (
                <BillingPaymentIssueBanner
                    ...
                    onResolve={() => void handleResolveBillingPaymentIssue()}
                />
            )}
   ...
   const retryStudyFocusLoad = React.useCallback(async () => {
      ...
      try {
         ...
         setStudyFocusLoadStatus('ready');
      } catch (error) {
         clientLog.warn('profile.study_focus_taxonomies.retry_failed', error);
         setStudyFocusLoadStatus('error');
      }
        ...
        <div className="mx-auto w-full max-w-7xl px-3 sm:px-4 md:px-6 pb-20 space-y-5 md:space-y-6">
            ...
            <main className="lg:col-span-9">
               ...
               {activeTab === 'saved-questions' && (
                  <SavedQuestionsPanel
                     rows={savedQuestionRows}
                     total={savedQuestionTotal}
                     answeredCount={savedAnsweredCount}
                     isLoading={isLoadingSavedQuestions}
                     answers={userAnswers}
                     hasNextPage={Boolean(savedQuestionsQuery.hasNextPage)}
                     isFetchingNextPage={savedQuestionsQuery.isFetchingNextPage}
                     loadMore={() => { void savedQuestionsQuery.fetchNextPage(); }}
                     navigate={(href) => router.push(href)}
                     remove={handleRemoveSavedQuestion}
                  />
               )}
                     ...
                     >
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="relative group/exam">
                                 <input name="targetExam" type="hidden" value={personalTargetExam} />
                                 <button type="button" onClick={() => setShowGoalModal(true)} aria-haspopup="dialog" aria-expanded={showGoalModal} className="w-full h-11 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 pr-10 font-bold text-sm text-left text-slate-900 dark:text-slate-100 outline-none cursor-pointer hover:border-indigo-300 dark:hover:border-indigo-900 transition-colors font-sans">
                                    {personalTargetExam || <span className="text-slate-500 dark:text-slate-400">Selecione seu foco</span>}
                                 </button>
                                 <div className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 group-hover/exam:text-indigo-500 transition-colors">
