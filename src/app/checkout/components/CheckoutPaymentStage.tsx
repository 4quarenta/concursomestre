...
const BillingField: React.FC<{
  ...
  <div>
    ...
    <div className="flex min-h-[50px] items-center gap-3 rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 dark:border-slate-700 dark:bg-[#111428] dark:text-slate-100">
    ...
    <section>
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-lg font-medium text-slate-950 dark:text-white">Dados confirmados</h2>
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-700 transition-colors hover:bg-slate-50 dark:border-slate-700 dark:bg-[#111428] dark:text-slate-200 dark:hover:bg-slate-800/50"
        >
          <PencilLine size={16} />
          Editar
        </button>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-[#111428]">
        <div className="mb-5 flex flex-wrap items-center gap-2">
          ...
          <span className={`inline-flex rounded-full px-3 py-1 text-xs font-medium ${
            emailVerified ? 'bg-slate-100 text-slate-700 dark:bg-slate-700/40 dark:text-slate-200' : 'bg-amber-100 text-amber-700'
          }`}>
        ...
        <div className="mt-4 flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-900">
          ...
          <div className="space-y-3">
            ...
            <button
              type="button"
              onClick={onEditBillingInfo}
              className="rounded-lg bg-slate-900 px-4 py-2 text-xs font-medium text-white transition-colors hover:bg-slate-800 dark:bg-white dark:text-slate-950 dark:hover:bg-slate-100"
            >
              Completar agora
            </button>
          </div>
        ...
        <div className="space-y-5">
          {!hasEnabledMethods ? (
            ...
            <>
              ...
              {isLoadingStripeCards ? (
                <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600 dark:border-slate-700 dark:bg-[#111428] dark:text-slate-300">
                    ...
                    <button
                      ...
                      className={`w-full rounded-lg border p-4 text-left transition-colors ${
                        isSelected
                          ...
                          : 'border-slate-200 bg-white hover:border-slate-300 dark:border-slate-700 dark:bg-[#111428] dark:hover:border-slate-600'
                      }`}
                    >
                      <div className="flex items-center justify-between gap-3">
                        ...
                        {isSelected ? <CheckCircle2 size={18} className="text-slate-900 dark:text-indigo-300" /> : null}
                      </div>
                    </button>
                  );
                })}
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-[#111428]">
              <div className="flex flex-col items-center py-6 text-center">
                <div className="mb-5 rounded-xl border border-slate-200 bg-white p-8 dark:border-slate-700 dark:bg-[#0f1020]">
                  <QrCode className="size-28 text-slate-900 dark:text-slate-100" />
                </div>
              ...
              <div className="rounded-lg bg-slate-50 p-4 dark:bg-[#0f1020]">
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button
                    type="button"
                    disabled
                    className="inline-flex shrink-0 items-center gap-2 rounded-md bg-slate-300 px-3 py-1.5 text-xs text-white dark:bg-slate-700"
                  >
                    <Copy size={14} />
                    Copiar
                  </button>
          ...
          {isCardFlow && !usingSavedCard ? (
            <label className="flex items-start gap-3 rounded-lg border border-slate-200 p-4 dark:border-slate-700">
              <input
                ...
                className="mt-0.5 size-4 cursor-pointer accent-slate-900 dark:accent-indigo-400"
              />
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-[#111428]">
            <button
              type="button"
              onClick={() => {
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <input
                      ...
                      className="w-full rounded-lg border border-slate-200 px-4 py-2.5 pl-10 text-sm uppercase text-slate-900 focus:border-slate-900 focus:outline-none focus:ring-1 focus:ring-slate-900 dark:border-slate-700 dark:bg-[#0f1020] dark:text-white dark:focus:border-indigo-400 dark:focus:ring-indigo-500/20"
                    />
                    <Tag className="absolute left-3 top-3 size-4 text-slate-400 dark:text-slate-500" />
                  </div>
                  <button
                    type="button"
                    onClick={onApplyCoupon}
                    disabled={applyingCoupon || !couponCode.trim()}
                    className="rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-slate-800 disabled:bg-slate-300 dark:bg-white dark:text-slate-950 dark:hover:bg-slate-100 dark:disabled:bg-slate-700"
                  >
            ...
            {appliedCouponCode ? (
              <div className="mt-4 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 dark:border-emerald-500/30 dark:bg-emerald-500/10">
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl shadow-slate-200/60 dark:border-slate-800 dark:bg-[#1a1c2e] dark:shadow-none">
        <div className="relative overflow-hidden bg-slate-950 p-5 sm:p-6 text-white">
          ...
          <div className="relative z-10">
            ...
            <h2 className="mt-3 text-2xl sm:text-3xl font-black tracking-tight">{planName}</h2>
        ...
        <div className="p-4 sm:p-6">
          ...
          <div className="mb-6 rounded-lg bg-slate-50 p-4 dark:bg-[#0f1020]">
            <ul className="space-y-2">
              {planBenefits.slice(0, 3).map((benefit) => (
                <li key={benefit} className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
                  <Check className="size-4 shrink-0 text-slate-900 dark:text-slate-100" />
                  <span>{benefit}</span>
                </li>
              ))}
            </ul>
          ...
          <div className="space-y-3 border-t border-slate-200 pt-4 dark:border-slate-700">
            <div className="flex justify-between gap-4 text-sm">
              <span className="text-slate-600 dark:text-slate-400">Forma de pagamento</span>
              <span className="font-semibold text-slate-950 dark:text-slate-100">{paymentMethodLabel}</span>
            </div>
            <div className="flex justify-between gap-4 text-sm">
              <span className="text-slate-600 dark:text-slate-400">Parcelamento</span>
              <span className="font-semibold text-slate-950 dark:text-slate-100">{installmentSummaryLabel}</span>
            </div>
            {nextRenewalLabel ? (
              <div className="flex justify-between gap-4 text-sm">
                <span className="text-slate-600 dark:text-slate-400">Renovação</span>
                <span className="font-semibold text-slate-950 dark:text-slate-100">{nextRenewalLabel}</span>
              </div>
            ) : null}
            <div className="flex justify-between gap-4 text-sm">
              <span className="text-slate-600 dark:text-slate-400">Subtotal</span>
              <span className="font-semibold text-slate-950 dark:text-slate-100">{subtotalLabel}</span>
            </div>
            {couponDiscountLabel ? (
              <div className="flex justify-between gap-4 text-sm">
                <span className="text-emerald-600">Desconto cupom</span>
                <span className="font-semibold text-emerald-600">- {couponDiscountLabel}</span>
              </div>
            ) : null}
            {residualCreditDiscountLabel ? (
              <div className="flex justify-between gap-4 text-sm">
                <span className="text-emerald-600">Desconto saldo</span>
                <span className="font-semibold text-emerald-600">- {residualCreditDiscountLabel}</span>
              </div>
            ) : null}
            <div className="flex justify-between gap-4 border-t border-slate-200 pt-3 text-sm dark:border-slate-700">
              <span className="font-black text-slate-700 dark:text-slate-300">TOTAL:</span>
              <span className="font-black text-slate-950 dark:text-white">{totalLabel}</span>
            </div>
          </div>

          <div className="mt-4 border-t border-slate-200 pt-4 dark:border-slate-700">
            <div className="flex items-baseline justify-between gap-3">
              <span className="text-slate-600 dark:text-slate-400">Total hoje</span>
              <span className="text-2xl sm:text-3xl font-black tracking-tight text-indigo-600">{dueLabel}</span>
            </div>
            {paymentProtectionLabel ? (
        ...
        <div className="mt-6 border-t border-slate-200 bg-white pt-6 dark:border-slate-800 dark:bg-[#1a1c2e]">
          <button
            type="button"
            onClick={onConfirmClick}
            disabled={processing || !paymentReady}
