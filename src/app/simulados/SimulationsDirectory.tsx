...
export default async function SimulationsDirectory({ searchParams }: { searchParams: Promise<{ pagina?: string; busca?: string }> }) {
  ...
  const structuredData = { '@context': 'https://schema.org', '@graph': [
    { '@type': 'CollectionPage', '@id': `${buildSiteUrl(path)}#webpage`, url: buildSiteUrl(path), name: 'Simulados', description },
    { '@type': 'BreadcrumbList', itemListElement: [{ '@type': 'ListItem', position: 1, name: 'Início', item: buildSiteUrl('/') }, { '@type': 'ListItem', position: 2, name: 'Simulados', item: buildSiteUrl(path) }] },
  ...
  return <article data-semantic-content className="w-full space-y-5 animate-fade-in">
    ...
    <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 text-xs font-bold text-slate-500"><Link href="/">Início</Link><ChevronRight size={12}/><span aria-current="page">Simulados</span></nav>
