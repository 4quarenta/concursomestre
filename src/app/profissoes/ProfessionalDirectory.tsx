...
export default async function ProfessionalDirectory({ kind, searchParams }: { kind: ProfessionalKind; searchParams: Promise<{ pagina?: string; busca?: string; letra?: string }> }) {
  ...
  const career = kind === 'career'; const path = career ? publicRoutes.careers.index() : publicRoutes.positions.index(); const title = career ? 'Carreiras' : 'Cargos';
  ...
  const breadcrumbs = [{ label: 'Início', path: '/' }, { label: title, path }];
  const structuredData = buildStructuredDataGraph([
    buildCollectionPage({ path, name: title, description }),
    buildBreadcrumbList(breadcrumbs),
    buildItemList(directory.items.map((item) => ({ name: item.name, path: item.path }))),
  ]);
