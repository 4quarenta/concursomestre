...
import {
  buildLawSections,
  isArticleNumberWithinRange,
  type LawSectionEditorial,
...
type ReadingTab = 'resumo' | 'comentarios' | 'questoes';
...
const LawDetailPage: React.FC = () => {
  ...
  const sectionQueryId = String(searchParams.get('section') || '').trim();
  ...
  const [activeTab, setActiveTab] = React.useState<ReadingTab>('comentarios');
  ...
  const sections = React.useMemo(() => {
    if (!law) return [];
    const parsed = buildLawSections(law.articles || [], [law.id, law.slug, law.catalogId, law.shortTitle, law.number]);
    if (parsed.length > 0) {
      return parsed;
    }
    ...
    return [{
      ...
      fromArticle: String(firstArticle?.number || firstArticle?.numero || '1'),
      toArticle: String(lastArticle?.number || lastArticle?.numero || law.articleCount || '1'),
      articles: Math.max(Number(law.articleCount || law.articles.length || 1), 1),
      primaryArticleId: String(firstArticle?.id || ''),
      articleIds: law.articles.map((article) => String(article.id || '')).filter(Boolean),
      isFavorite: false,
    }];
  }, [law]);
  ...
  const activeSection = React.useMemo<LawSectionSummary | null>(() => {
    ...
    if (sectionQueryId) {
      const byId = sections.find((section) => String(section.id) === sectionQueryId);
      if (byId) return byId;
    }
  ...
  const activeSectionArticles = React.useMemo(() => {
    ...
    const sectionArticleIds = activeSection.articleIds?.length
      ...
      : null;
    const rangeFrom = activeSection.fromArticle;
    const rangeTo = activeSection.toArticle;
    ...
    return law.articles.filter((article) => {
      ...
      return isArticleNumberWithinRange(article.number || article.numero, rangeFrom, rangeTo);
    });
  }, [activeSection, law]);
  ...
  const progressPercent = Number(law?.progress?.progressPercent || law?.progressPercent || 0);
  ...
  const toggleLawFavorite = React.useCallback(async () => {
    if (!law || isTogglingFavorite) return;
    if (!userId) {
      addToast('Entre na sua conta para favoritar leis.', 'warning');
      return;
    }

    setIsTogglingFavorite(true);
    const optimistic = !law.isFavorite;
    setLaw((current) => (current ? { ...current, isFavorite: optimistic } : current));

    try {
      const result = await legalCommentaryApiService.toggleFavorite('law', law.id);
      setLaw((current) => (current ? { ...current, isFavorite: Boolean(result.isFavorite) } : current));
  ...
  }, [addToast, isTogglingFavorite, law, userId]);
    ...
    <div className="space-y-4 pb-10">
      <section className="sticky top-2 z-30">
        <div className={`${PLATFORM_SURFACE_CARD_CLASS} flex flex-wrap items-center gap-2 px-3 py-2`}>
          <ToolbarButton icon={<Table size={14} />} label="Sumário" />
          <ToolbarButton icon={<FileText size={14} />} label="Texto" active />
          <div className="inline-flex h-9 items-center gap-2 rounded-lg border border-slate-200 bg-white px-2 dark:border-slate-700 dark:bg-slate-900">
            <button
              type="button"
              onClick={() => setFontScale((value) => Math.max(90, value - 5))}
              className="grid h-7 w-7 place-items-center rounded-md text-slate-500 transition-colors hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800"
            >
              <Minus size={14} />
            </button>
          ...
          <ToolbarButton icon={<Eye size={14} />} label="Modo foco" active />
          <ToolbarButton icon={<Bookmark size={14} />} label="Marcar revisão" onClick={() => addToast('Seção marcada para revisão.', 'success')} />
          <ToolbarButton icon={<Star size={14} />} label="Favoritar" onClick={toggleLawFavorite} active={Boolean(law.isFavorite)} />
          <ToolbarButton icon={<FileText size={14} />} label="PDF" onClick={() => window.open(law.officialUrl, '_blank', 'noopener,noreferrer')} />
          <ToolbarButton icon={<ChevronDown size={14} />} label="Mais" />
        </div>
      </section>
      ...
      <section className={`${PLATFORM_SURFACE_CARD_CLASS} p-5`}>
        ...
        <div className="mt-3 flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <ToolbarButton icon={<Star size={14} />} label="Favoritar" onClick={toggleLawFavorite} active={Boolean(law.isFavorite)} />
            <ToolbarButton icon={<Save size={14} />} label="Salvar" onClick={() => addToast('Progresso salvo.', 'success')} />
      ...
      <section className={`${PLATFORM_SURFACE_CARD_CLASS} p-1`}>
      ...
      <section className={`${PLATFORM_SURFACE_CARD_CLASS} p-3`}>
        <div className="flex flex-wrap items-center gap-2">
          <button type="button" className="inline-flex h-8 items-center justify-center rounded-lg border border-slate-200 bg-white px-2 text-xs font-bold text-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200">16px</button>
          <button type="button" className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200">B</button>
          <button type="button" className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200">I</button>
          <button type="button" className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200">U</button>
          <button type="button" className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"><Highlighter size={14} /></button>
          <button type="button" className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"><CircleHelp size={14} /></button>
          <div className="ml-auto">
            <button
              type="button"
              onClick={() => addToast('Anotações de leitura salvas.', 'success')}
              className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-slate-900 px-3 text-xs font-bold text-white transition-colors hover:bg-slate-800 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-white"
            >
              <Save size={13} />
              Salvar
            </button>
      ...
      {activeTab === 'questoes' ? (
        <section className={`${PLATFORM_SURFACE_CARD_CLASS} p-6`}>
          ...
          <p className="mt-2 text-sm font-medium text-slate-500 dark:text-slate-300">
            Esta faixa possui {totalRelatedQuestions} questões vinculadas para prática.
          </p>
          <div className="mt-4">
            <Link
              ...
              <div className="mx-auto max-w-[980px] rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900 md:p-9">
                ...
                <div className="space-y-6">
                  {visibleArticles.map((article) => {
                    ...
                    const inlineNotesByBlock = activeTab === 'comentarios'
