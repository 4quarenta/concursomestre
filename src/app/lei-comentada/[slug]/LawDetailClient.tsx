...
const LawDetailPage: React.FC<LawDetailPageProps> = ({
  ...
  const serverLawPendingRef = React.useRef(Boolean(initialLaw));
  ...
  React.useEffect(() => {
    if (serverLawPendingRef.current) {
      serverLawPendingRef.current = false;

      if (!userId && !requestedLawId) {
        setIsLoading(false);
        setLoadError('');
        return undefined;
      }
