...
const AnnotatedLawsPage: React.FC<AnnotatedLawsPageProps> = ({ initialSnapshot = null }) => {
  ...
  const [snapshot, setSnapshot] = React.useState<LegalHomeSnapshot>(initialSnapshot || EMPTY_LEGAL_HOME);
  ...
  const [lawFavoriteBusyMap, setLawFavoriteBusyMap] = React.useState<Record<string, boolean>>({});
  ...
  const serverSnapshotPendingRef = React.useRef(Boolean(initialSnapshot));
  ...
  React.useEffect(() => {
    if (serverSnapshotPendingRef.current) {
      serverSnapshotPendingRef.current = false;

      if (!userId && homeReloadVersion === 0) {
        setIsLoading(false);
  ...
  const handleToggleSectionFavorite = React.useCallback(async (
    ...
    try {
      const result = await legalCommentaryApiService.toggleFavorite('section', targetSectionId);
  ...
  const handleToggleLawFavorite = React.useCallback(async (law: LawSummary) => {
    const lawId = String(law.id || '').trim();
    if (!lawId || lawFavoriteBusyMap[lawId]) return;
    if (!userId) {
    ...
    const previousFavorite = Boolean(law.isFavorite);
    const updateFavoriteState = (isFavorite: boolean) => {
      setSnapshot((current) => {
        const updateLaw = (item: LawSummary) => (
          String(item.id) === lawId ? { ...item, isFavorite } : item
        );
        const updatedLaw = { ...law, isFavorite };
        const favoriteLaws = isFavorite
          ? [updatedLaw, ...current.favoriteLaws.filter((item) => String(item.id) !== lawId)]
          : current.favoriteLaws.filter((item) => String(item.id) !== lawId);

        return {
          ...current,
          lawsByArea: current.lawsByArea.map((group) => ({
            ...group,
            laws: group.laws.map(updateLaw),
          })),
          mostAccessed: current.mostAccessed.map(updateLaw),
          favoriteLaws,
          recentlyStudied: current.recentlyStudied.map(updateLaw),
          recentlyUpdated: current.recentlyUpdated.map(updateLaw),
        };
      });
    };

    setLawFavoriteBusyMap((current) => ({ ...current, [lawId]: true }));
    updateFavoriteState(!previousFavorite);

    try {
      const result = await legalCommentaryApiService.toggleFavorite('law', lawId);
      updateFavoriteState(Boolean(result.isFavorite));
      addToast(result.isFavorite ? 'Lei adicionada aos favoritos.' : 'Lei removida dos favoritos.', 'success');
    } catch {
      updateFavoriteState(previousFavorite);
      addToast('Não foi possível atualizar o favorito desta lei.', 'error');
    } finally {
      setLawFavoriteBusyMap((current) => {
        const next = { ...current };
        delete next[lawId];
        return next;
      });
    }
  }, [addToast, canUseLegalFavorites, lawFavoriteBusyMap, userId]);
                      ...
                      {group.laws.map((law) => {
                        ...
                        return (
                          <article key={law.id} className="overflow-hidden border-b border-slate-100 last:border-b-0 dark:border-slate-800">
                            <div className="flex w-full items-stretch gap-2 px-4 py-3 transition-colors hover:bg-slate-50/70 dark:hover:bg-slate-800/40">
                              <button
                                type="button"
                              ...
                              <button
                                type="button"
                                onClick={() => {
                                  void handleToggleLawFavorite(law);
                                }}
                                disabled={Boolean(lawFavoriteBusyMap[String(law.id)])}
                                className="inline-flex h-9 w-9 shrink-0 self-center items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-400 transition-colors hover:border-amber-200 hover:text-amber-500 disabled:cursor-not-allowed disabled:opacity-60 dark:border-slate-700 dark:bg-slate-900"
                                title={!canUseLegalFavorites ? 'Favoritos disponíveis em plano superior' : law.isFavorite ? 'Remover lei dos favoritos' : 'Salvar lei completa'}
                                aria-label={!canUseLegalFavorites ? 'Favoritos bloqueados para este plano' : law.isFavorite ? 'Remover lei dos favoritos' : 'Salvar lei completa'}
                              >
                                {canUseLegalFavorites ? (
                                  <Bookmark size={16} className={law.isFavorite ? 'fill-amber-400 text-amber-500' : ''} />
                                ) : (
