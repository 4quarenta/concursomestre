...
export default async function MaterialsDirectory({ searchParams }: { searchParams: Promise<{ pagina?: string; busca?: string }> }) {
  const params = await searchParams; const directory = await fetchPublicMaterialDirectory(params); const search = String(params.busca || '').trim(); const path = publicRoutes.materials.index();
