...
const Dashboard: React.FC = () => {
  ...
  // Filtrar respostas com base no perÃ­odo selecionado
  ...
  // CÃ¡lculo de comentÃ¡rios feitos pelo usuÃ¡rio no perÃ­odo
  ...
  // CÃ¡lculo da evoluÃ§Ã£o temporal
  const timelineData = useMemo(() => {
    ...
    filteredAnswers.forEach(ans => {
      ...
      if (data[label]) {
        data[label].total++;
        // Re-calcular mÃ©dia incremental (nÃ£o o melhor, mas ok para o volume)
        // No entanto, para precisÃ£o real no perÃ­odo, vamos armazenar os acertos
      }
    });
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100">VisÃ£o Geral</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-2">Acompanhe sua evoluÃ§Ã£o e identifique pontos de melhoria.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Taxa de Acerto" value={`${accuracy}%`} subtext="No perÃ­odo selecionado" icon={Target} color="bg-indigo-500" />
        <StatCard title="QuestÃµes Feitas" value={totalQuestions} subtext="No perÃ­odo selecionado" icon={Zap} color="bg-blue-500" />
        <StatCard title="ComentÃ¡rios" value={userCommentsCount} subtext="ParticipaÃ§Ã£o ativa" icon={MessageSquare} color="bg-amber-500" />
        <StatCard title="NÃ­vel Atual" value={currentUser?.level || 1} subtext="Sua progressÃ£o" icon={Trophy} color="bg-purple-500" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm min-h-[350px] transition-colors duration-300 lg:col-span-2">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
              <TrendingUp size={18} className="text-indigo-500" /> EvoluÃ§Ã£o do Desempenho
            </h3>
            <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">
              <span className="w-2 h-2 rounded-full bg-indigo-500" /> PrecisÃ£o (%)
            </div>
          </div>
          <div className="h-64 w-full min-w-0 overflow-hidden">
            <ResponsiveContainer width="100%" height="100%" minWidth={0}>
              <AreaChart data={timelineData}>
                ...
                />
                <Area type="monotone" dataKey="total" stroke="#f59e0b" strokeWidth={3} fillOpacity={1} fill="url(#colorTotal)" name="QuestÃµes" />
                <Area type="monotone" dataKey="accuracy" stroke="#6366f1" strokeWidth={2} fillOpacity={0.1} fill="url(#colorAcc)" name="PrecisÃ£o (%)" />
                <defs>
          ...
          <div className="h-64 w-full min-w-0 overflow-hidden relative">
            {totalQuestions > 0 ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-slate-400">
                <p>Nenhuma questÃ£o respondida neste perÃ­odo.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm min-h-[300px] transition-colors duration-300">
          <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-6 flex items-center gap-2">
            <BarChart3 size={18} className="text-slate-400 dark:text-slate-500" /> Desempenho por MatÃ©ria
          </h3>
          <div className="h-64 w-full min-w-0 overflow-hidden">
