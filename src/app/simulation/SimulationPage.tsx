...
import {
   ...
   Calendar, Building2, Briefcase, GraduationCap, BookOpen, List, Eye, LucideIcon, Maximize2, Minimize2
} from 'lucide-react';
...
const Simulation: React.FC = () => {
   ...
   const enterFullscreenMode = React.useCallback(() => {
      const fullscreenRequest = requestSimulationFullscreen(document);
      updateImmersiveMode(true);
      void fullscreenRequest.then((entered) => {
         setIsNativeFullscreen(entered || Boolean(getSimulationFullscreenElement(document)));
      });
   }, [updateImmersiveMode]);

   const leaveFullscreenMode = React.useCallback(() => {
      updateImmersiveMode(false);
      void exitSimulationFullscreen(document).then(() => {
         setIsNativeFullscreen(Boolean(getSimulationFullscreenElement(document)));
      });
   }, [updateImmersiveMode]);

   const isFullscreenActive = isImmersiveEnabled || isNativeFullscreen;

   useEffect(() => {
      const syncFullscreenState = () => {
         const active = Boolean(getSimulationFullscreenElement(document));
         setIsNativeFullscreen(active);

         if (active !== isImmersiveEnabled) {
            updateImmersiveMode(active);
         }
      };
   ...
   }, [isImmersiveEnabled, updateImmersiveMode]);
            ...
            <div className="sticky top-2 md:top-4 z-40 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border border-slate-200 dark:border-slate-800 rounded-2xl p-3 sm:p-4 mb-6 md:mb-8 flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center shadow-lg transition-colors">
               ...
               <div className="flex w-full sm:w-auto items-center justify-between gap-3 sm:gap-4">
                  ...
                  <div className="flex items-center gap-2">
                     <button
                        onClick={isFullscreenActive ? leaveFullscreenMode : enterFullscreenMode}
                        ...
                        title={isFullscreenActive ? 'Sair da tela cheia' : 'Entrar em tela cheia'}
                     >
                        {isFullscreenActive ? <Minimize2 size={13} /> : <Maximize2 size={13} />}
            ...
            <div className="sticky top-2 md:top-4 z-40 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border border-slate-200 dark:border-slate-800 rounded-2xl p-3 sm:p-4 mb-6 md:mb-8 flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center shadow-lg transition-colors">
               ...
               <div className="flex gap-2 sm:justify-end">
                  <button
                     ...
                     title={isFullscreenActive ? 'Sair da tela cheia' : 'Entrar em tela cheia'}
                  >
                     {isFullscreenActive ? <Minimize2 size={12} /> : <Maximize2 size={12} />}
                     {isFullscreenActive ? 'Sair' : 'Tela cheia'}
                  </button>
