...
export type PublicContest = {
  ...
  positions: Array<{ id: number; roleId: number; name: string; vacancies: number | null; reserveRegistry: boolean; salaryMin: number | null; salaryMax: number | null; educationLevel: string | null; weeklyHours: number | null; locationLabel: string | null }>;
...
export const parsePublicContest = (value: unknown): PublicContest | { redirectSlug: string } | null => {
  ...
  return {
    ...
    positions: list(value.positions, (item) => ({ id: int(item.id), roleId: int(item.roleId), name: text(item.name), vacancies: item.vacancies == null ? null : int(item.vacancies), reserveRegistry: Boolean(item.reserveRegistry), salaryMin: item.salaryMin == null ? null : Number(item.salaryMin), salaryMax: item.salaryMax == null ? null : Number(item.salaryMax), educationLevel: nullableText(item.educationLevel), weeklyHours: item.weeklyHours == null ? null : int(item.weeklyHours), locationLabel: nullableText(item.locationLabel) })),
