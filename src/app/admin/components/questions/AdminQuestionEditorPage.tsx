...
const normalizeQuestionGroupSelection = (
  ...
  return { ...group, id } as QuestionGroupSelection;
};
...
const AdminQuestionEditorPage = ({
  ...
  const handleOptionImageRemove = (index: number) => {
    setManualQ((prev) => {
      ...
      return {
        ...prev,
        assets: (prev.assets || []).filter((asset) => !asset.id || !markerIds.includes(asset.id)),
        itens: nextItems,
      };
    });
  };
                      ...
                      {filteredQuestionGroups.length > 0 ? filteredQuestionGroups.map((group) => {
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            onClick={() => {
                              const normalized = normalizeQuestionGroupSelection(group);
                              if (normalized) {
                                handleSelectQuestionGroup(normalized);
                              }
                            }}
