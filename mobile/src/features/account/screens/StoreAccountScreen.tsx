...
export const StoreAccountScreen: React.FC = () => {
  ...
  const changePassword = async () => {
    ...
    if (newPassword.length < 6) {
      Alert.alert('Seguranca', 'A nova senha precisa ter pelo menos 6 caracteres.');
      return;
    }
        ...
        { text: 'Voltar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert('Assinatura', result.message || 'Renovacao automatica desativada.');
            } catch (error: any) {
  ...
  const confirmLogout = () => Alert.alert(
    'Sair da conta?',
    'Sua sessao neste aparelho sera encerrada.',
    [
      { text: 'Cancelar', style: 'cancel' },
    ...
    >
      ...
      <View style={styles.card}>
        ...
        <View style={styles.planCard}>
          ...
          {subscription?.current_period_end ? (
            <Text style={styles.muted}>Ciclo atual ate {formatDate(subscription.current_period_end)}</Text>
          ) : null}
          {subscription ? (
            <Text style={styles.muted}>Renovacao automatica: {renewalEnabled ? 'ativada' : 'desativada'}</Text>
          ) : null}
        </View>
        <Text style={styles.helper}>
          Esta versao para loja reconhece o acesso associado a sua conta, mas nao inicia compras, upgrades ou pagamentos externos.
        </Text>
        {subscription && renewalEnabled ? (
          ...
          >
            {cancellingRenewal ? (
              ...
              <Text style={styles.dangerText}>Desativar renovacao automatica</Text>
            )}
          </Pressable>
        ) : null}
      </View>
      ...
      <View style={styles.card}>
        ...
        <Pressable
          ...
          style={[styles.primaryButtonWide, changingPassword && styles.disabled]}
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.helper}>
          O pedido de exclusao e registrado no servidor e exige confirmacao da senha atual. O app encerra a sessao apos o pedido.
        </Text>
        <TextInput
          secureTextEntry
      ...
      <View style={styles.card}>
        ...
        <Pressable onPress={() => void openPublicLink(PUBLIC_LINKS.privacy)} style={styles.secondaryButtonWide}>
          <Text style={styles.secondaryText}>Politica de Privacidade</Text>
        </Pressable>
        <Pressable onPress={() => void openPublicLink(PUBLIC_LINKS.terms)} style={styles.secondaryButtonWide}>
          <Text style={styles.secondaryText}>Termos de Uso</Text>
        </Pressable>
        <Pressable onPress={() => void openPublicLink(PUBLIC_LINKS.support)} style={styles.secondaryButtonWide}>
          <Text style={styles.secondaryText}>Suporte</Text>
        </Pressable>
        <Pressable onPress={() => void openPublicLink(PUBLIC_LINKS.accountDeletion)} style={styles.secondaryButtonWide}>
