...
import { Question, UserAnswer, QuestaoComentario as Comment, ErrorReport, UserNote, QuestionStats } from '../types';
import {
...
import { apiClient, ENDPOINTS, getAssetUrl } from '@core/api';
...
const QuestionCard: React.FC<QuestionCardProps> = ({
  ...
  const handleToggleStats = async () => {
    ...
    if (nextState) {
      ...
      try {
        const res = await apiClient.get<any>(ENDPOINTS.questions.stats, {
          params: { question_id: String(question.id) }
        });
  ...
  useEffect(() => {
    if (isHistoryModalOpen) {
      ...
      apiClient.get<UserAnswer[]>(ENDPOINTS.questions.history, {
        params: { question_id: String(question.id), user_id: currentUser?.id || '' }
      })
        .then(res => {
          const data = (res as any).data || res;
