...
export const PaymentModal: React.FC<PaymentModalProps> = ({
    ...
    const updateInstallments = async (bin: string) => {
        try {
            const response = await axios.get(`http://localhost/questao-pro-backend/api/payments/get-installments.php`, {
                params: { amount: material.price, bin }
            });
            if (response.data && response.data[0]) {
                setInstallmentOptions(response.data[0].payer_costs || []);
                setPaymentMethodId(response.data[0].payment_method_id || 'master');
            }
        } catch (err) {
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            // 4. Send to backend
            const response = await axios.post('http://localhost/questao-pro-backend/api/payments/process-payment.php', {
            ...
            if (response.data.status === 'approved') {
                ...
                setError(response.data.message || 'Pagamento recusado ou erro no processamento.');
            }
        } catch (err: any) {
            console.error('Payment error:', err);
            setError(err.message || err.response?.data?.message || 'Erro de conexão com o servidor.');
        } finally {
            setIsLoading(false);
        }
    };
