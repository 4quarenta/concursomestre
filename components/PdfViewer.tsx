...
import { apiClient, ENDPOINTS } from '../src/core/api'; // Ensure this path is correct based on project structure
...
const PdfViewer: React.FC<PdfViewerProps> = ({ url, isOpen, onClose, title, mode = 'modal', materialId, password }) => {
    ...
    const fetchNote = async () => {
        if (!materialId || !currentUser) return;
        try {
            const res = await apiClient.get<any>(`/materials/get_note.php`, {
                params: { user_id: currentUser.id, material_id: materialId }
            });
            const data = (res as any).data || res; // Handle axios response structure
            if (data.success && data.note) {
                setNoteText(data.note.note_text);
                setOriginalNoteText(data.note.note_text);
            }
        } catch (err) {
    ...
    const fetchBookmarks = async () => {
        if (!materialId || !currentUser) return;
        try {
            const res = await apiClient.get<any>(`/materials/get_bookmarks.php`, {
                params: { user_id: currentUser.id, material_id: materialId }
            });
    ...
    const saveBookmark = async () => {
        ...
        try {
            const res = await apiClient.post<any>(`/materials/save_bookmark.php`, {
    ...
    const deleteBookmark = async (id: number) => {
        try {
            await apiClient.delete(`/materials/delete_bookmark.php?id=${id}`);
    ...
    const handleSaveNote = async () => {
        ...
        setSavingNote(true);
        try {
            await apiClient.post(`/materials/save_note.php`, {
                user_id: currentUser.id,
                material_id: materialId,
                note_text: noteText
            });
