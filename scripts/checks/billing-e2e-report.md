...
| --- | --- | --- | --- |
| Renovacao Stripe ponta a ponta | OK | Stripe Test Clock + createStripeInlineSubscription + finalizeStripeSubscription + runStripeReconciliationCron. | subscription=sub_1TJlTsHTtB22su0x7Lqk5iZl<br>initial_invoice=in_1TJlTsHTtB22su0xz7Kmih5J<br>renewal_invoice=in_1TJlU6HTtB22su0xRgorFujb<br>transactions_after=2<br>synced_period_end=2026-06-07 23:04:29 |
| Auto renew off/on com backend como fonte final | OK | SubscriptionsService::updateRenewal + leitura remota da Stripe + verificacao de banco local. | off_message=Renovacao automatica desativada. A assinatura sera encerrada ao fim do periodo atual.<br>missing_card_error=Nenhum cartao padrao foi encontrado no Stripe para religar a renovacao automatica.<br>on_message=Renovacao automatica ativada com sucesso.<br>payment_method=pm_1TJlToHTtB22su0xcdaFPBCU |
| Upgrade e pro-rata canonico | OK | createStripeInlineSubscription + calculateSafeProratedCredit + persistencia de transacoes locais. | credit=19.33<br>coupon=E2EUP5FB55<br>upgrade_final=5.67<br>local_credit_mode=local_credit |
...
| Webhook duplicado | OK | Fixture invoice.paid + processStripeWebhookEventObject com Stripe real e banco local. | event_id=evt_dup_dc14b4c6<br>first_duplicate={"received":true}<br>second_duplicate={"received":true,"duplicate":true}<br>transactions=1 |
...
| Webhook atrasado + reconciliacao | OK | Stripe Test Clock + runStripeReconciliationCron + fixture invoice.paid atrasado. | renewal_invoice=in_1TJlVjHTtB22su0xOl21y7lB<br>recovery_transactions=2<br>delayed_result={"received":true} |
